...
class RefundDeposit
{
    ...
    public function handle(RentalContract $contract, string $amount, string $idempotencyKey, int $actorId, ?string $reason = null): DepositTransaction
    {
        ...
        return DB::transaction(function () use ($contract, $minor, $idempotencyKey, $actorId, $reason) {
            ...
            $entry = DepositTransaction::create(['agency_id' => $locked->agency_id, 'rental_contract_id' => $locked->id, 'transaction_number' => $this->numbers->handle('deposit'), 'transaction_type' => 'refunded', 'amount' => DecimalMoney::fromMinorUnits($minor), 'currency' => $locked->currency, 'idempotency_key' => $idempotencyKey, 'occurred_at' => now(), 'reason' => $reason, 'created_by' => $actorId]);
