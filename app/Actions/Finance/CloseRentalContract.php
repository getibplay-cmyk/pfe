    ...
    public function handle(RentalContract $contract, int $actorId): RentalContract
    {
        return DB::transaction(function () use ($contract, $actorId) {
            ...
            if ($locked->status !== RentalContractStatus::Returned) {
                throw ValidationException::withMessages(['status' => __('Seul un contrat retourné peut être clôturé.')]);
            }
            if ($locked->charges()->where('status', ContractChargeStatus::Proposed)->exists()) {
                throw ValidationException::withMessages(['charges' => __('Tous les frais doivent être revus.')]);
            }
            if (! $locked->invoice || $locked->invoice->status !== 'paid' || $locked->invoice->balance_due !== '0.00') {
                throw ValidationException::withMessages(['invoice' => __('La facture émise doit être intégralement réglée.')]);
            }
            $totals = $this->ledger->syncContract($locked);
            if ($totals['balance'] !== 0) {
                throw ValidationException::withMessages(['deposit' => __('La caution doit être entièrement retenue ou remboursée.')]);
            }
            if ($locked->payments()->where('status', 'pending')->exists()) {
                throw ValidationException::withMessages(['payments' => __('Un paiement est encore en attente.')]);
            }
            ...
            ContractStatusHistory::create(['rental_contract_id' => $locked->id, 'from_status' => RentalContractStatus::Returned, 'to_status' => RentalContractStatus::Closed, 'reason' => __('Clôture financière'), 'changed_by' => $actorId]);
