...
class AllocatePaymentToInvoice
{
    ...
    public function handle(Payment $payment, Invoice $invoice, string $amount): PaymentAllocation
    {
        ...
        if ($amountMinor === 0) {
            throw ValidationException::withMessages(['amount' => __('Le montant alloué doit être positif.')]);
        }
        ...
        return DB::transaction(function () use ($payment, $invoice, $amountMinor) {
            ...
            if (! in_array($lockedPayment->status, ['pending', 'posted'], true) || $lockedPayment->direction !== 'incoming') {
                throw ValidationException::withMessages(['payment' => __('Le paiement entrant doit être en attente ou comptabilisé.')]);
            }
            if (! in_array($lockedInvoice->status, ['issued', 'partially_paid'], true)) {
                throw ValidationException::withMessages(['invoice' => __('La facture ne peut plus recevoir de règlement.')]);
            }
            if ($lockedPayment->tenant_id !== $lockedInvoice->tenant_id
                ...
                throw ValidationException::withMessages(['allocation' => __('Tenant, agence, client ou devise incompatible.')]);
            }
            ...
            if (DecimalMoney::toMinorUnits((string) $used) + $amountMinor > DecimalMoney::toMinorUnits($lockedPayment->amount)) {
                throw ValidationException::withMessages(['amount' => __('Le montant dépasse le disponible du paiement.')]);
            }
            ...
            if (DecimalMoney::toMinorUnits((string) $scheduled) + $amountMinor > DecimalMoney::toMinorUnits($lockedInvoice->total_amount)) {
                throw ValidationException::withMessages(['amount' => __('Le montant dépasse le solde disponible de la facture.')]);
            }
