...
class AcceptRentalContract
{
    ...
    public function handle(RentalContract $contract, array $data, ?int $actorId): RentalContract
    {
        return DB::transaction(function () use ($contract, $data, $actorId) {
            ...
            $version = ContractVersion::whereKey($locked->current_version_id)->lockForUpdate()->firstOrFail();
            ...
            if (! $driver || $driver->licence_expires_at->endOfDay()->lt($locked->expected_start_at)) {
                throw ValidationException::withMessages(['driver' => 'Le permis principal est absent ou expiré à la date de départ.']);
            }
            $required = config('rentals.required_document_types');
            $hasCustomer = Document::where('documentable_type', $locked->customer->getMorphClass())->where('documentable_id', $locked->customer_id)->where('document_type', $required[0])->exists();
            $hasLicence = Document::where('documentable_type', $driver->getMorphClass())->where('documentable_id', $driver->id)->where('document_type', $required[1])->exists();
            if (! $hasCustomer || ! $hasLicence) {
                throw ValidationException::withMessages(['documents' => 'Les documents d’identité client et de permis sont requis avant acceptation.']);
            }
