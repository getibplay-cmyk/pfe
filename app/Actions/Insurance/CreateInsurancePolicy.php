...
class CreateInsurancePolicy
{
    ...
    public function handle(array $data, int $actorId): InsurancePolicy
    {
        if (isset($data['status']) && (string) ($data['status'] instanceof InsurancePolicyStatus ? $data['status']->value : $data['status']) !== InsurancePolicyStatus::Draft->value) {
            throw ValidationException::withMessages(['status' => __('Une police doit toujours être créée en brouillon.')]);
        }
        if (! isset($data['policy_number']) || trim((string) $data['policy_number']) === '') {
            throw ValidationException::withMessages(['policy_number' => __('Le numéro de police est obligatoire.')]);
        }
        ...
        try {
            return DB::transaction(function () use ($normalized, $data, $actorId): InsurancePolicy {
                ...
                InsurancePolicyStatusHistory::create(['agency_id' => $policy->agency_id, 'insurance_policy_id' => $policy->id, 'from_status' => null, 'to_status' => InsurancePolicyStatus::Draft, 'reason' => __('Création de la police'), 'actor_id' => $actorId, 'changed_at' => now()]);
            ...
            if ($exception->getCode() === '23505') {
                throw ValidationException::withMessages(['policy_number' => __('Ce numéro de police est déjà utilisé dans l’entreprise.')]);
            }
            throw $exception;
        }
    }
