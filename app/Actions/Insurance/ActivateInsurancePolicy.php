        ...
        try {
            return DB::transaction(function () use ($policy, $actorId): InsurancePolicy {
                ...
                if ($locked->status !== InsurancePolicyStatus::Draft) {
                    throw ValidationException::withMessages(['policy' => __('Seule une police brouillon peut être activée.')]);
                }
                if (! $locked->company?->is_active) {
                    throw ValidationException::withMessages(['insurance_company_id' => __('La compagnie doit être active.')]);
                }
                if (! $locked->vehicle || $locked->vehicle->agency_id !== $locked->agency_id) {
                    throw ValidationException::withMessages(['vehicle_id' => __('Le véhicule n’appartient plus à l’agence de la police.')]);
                }
                if ($locked->ends_at->lessThan($locked->starts_at)) {
                    throw ValidationException::withMessages(['ends_at' => __('La période de la police est invalide.')]);
                }
                if (! $locked->coverages()->lockForUpdate()->exists()) {
                    throw ValidationException::withMessages(['coverage' => __('Au moins une garantie active est obligatoire.')]);
                }
                ...
                if ($document->tenant_id !== $locked->tenant_id || $document->agency_id !== $locked->agency_id) {
                    throw ValidationException::withMessages(['document' => __('Le document ne correspond pas au périmètre de la police.')]);
                }
                if (InsurancePolicy::query()->whereKeyNot($locked->id)->where('vehicle_id', $locked->vehicle_id)->where('policy_type', $locked->policy_type)->where('status', InsurancePolicyStatus::Active)->whereDate('starts_at', '<=', $locked->ends_at)->whereDate('ends_at', '>=', $locked->starts_at)->lockForUpdate()->exists()) {
                    throw ValidationException::withMessages(['period' => __('Une police active du même type couvre déjà ce véhicule sur cette période.')]);
                }
                ...
                InsurancePolicyStatusHistory::create(['agency_id' => $locked->agency_id, 'insurance_policy_id' => $locked->id, 'from_status' => InsurancePolicyStatus::Draft, 'to_status' => InsurancePolicyStatus::Active, 'reason' => __('Activation contrôlée'), 'actor_id' => $actorId, 'changed_at' => now()]);
