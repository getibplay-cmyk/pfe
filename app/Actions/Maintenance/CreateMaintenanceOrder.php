...
class CreateMaintenanceOrder
{
    ...
    public function handle(array $data, int $actorId): MaintenanceOrder
    {
        ...
        try {
            ...
            throw ValidationException::withMessages(['estimated_cost' => __('Le coût estimé doit être un montant décimal positif ou nul.')]);
        }
        ...
        if (($start === null) !== ($end === null) || ($start && $end && ($end->lessThanOrEqualTo($start) || $end->lessThanOrEqualTo(now())))) {
            throw ValidationException::withMessages(['scheduled_end_at' => __('La période doit être complète, ordonnée et se terminer dans le futur.')]);
        }
