...
class CustomerPortalAccessController extends Controller
{
    ...
    public function store(Request $request, Customer $customer)
    {
        ...
        if ($request->boolean('send_email') && (! app(CustomerPortalMail::class)->ready() || ! filter_var($customer->email, FILTER_VALIDATE_EMAIL))) {
            throw ValidationException::withMessages(['send_email' => __('Un service SMTP configuré et une adresse e-mail valide sur la fiche client sont nécessaires.')]);
        }
        abort_unless($customer->agency_id && Agency::whereKey($customer->agency_id)->where('is_active', true)->exists(), 422, __('Rattachez ce client à une agence active avant de créer un accès.'));
        $access = DB::transaction(function () use ($request, $customer) {
            ...
            if ($request->boolean('send_email') && ! filter_var($customer->email, FILTER_VALIDATE_EMAIL)) {
                throw ValidationException::withMessages(['send_email' => __('Renseignez une adresse e-mail valide sur la fiche client.')]);
            }
        ...
        if ($request->boolean('send_email')) {
            return redirect()->route('customers.portal-access.show', $customer)->with('status', __('L’envoi du lien a été mis en file d’attente.'));
        }
    ...
    public function revoke(Request $request, Customer $customer)
    {
        ...
        return back()->with('status', __('Tous les accès au portail de ce client ont été révoqués.'));
    }
    ...
    public function enter(Request $request, string $access)
    {
        ...
        abort_if($grant->consumed_at !== null, 403, __('Ce lien a déjà été utilisé.'));
    ...
    public function logout(Request $request)
    {
        ...
        return redirect()->route('home')->with('status', __('Vous avez quitté votre espace locataire.'));
    }
}
