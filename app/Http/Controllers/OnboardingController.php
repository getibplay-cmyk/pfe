...
class OnboardingController extends Controller
{
    public function __invoke(Request $request): View
    {
        ...
        $steps = collect([
            [
                'label' => __('Compléter l’entreprise'),
                'description' => __('Ajoutez la raison sociale, le téléphone et l’adresse.'),
                'complete' => filled($tenant->legal_name) && filled($tenant->phone) && filled($settings['address'] ?? null),
                'url' => route('tenant.show'),
                'action' => __('Compléter le profil'),
            ],
            [
                'label' => __('Vérifier l’agence initiale'),
                'description' => __('Contrôlez ses coordonnées et son adresse opérationnelle.'),
                ...
                'action' => __('Voir les agences'),
            ],
            [
                'label' => __('Créer une catégorie de véhicule'),
                'description' => __('Définissez une catégorie active avant de constituer le parc.'),
                ...
                'action' => __('Créer une catégorie'),
            ],
            [
                'label' => __('Ajouter le premier véhicule'),
                'description' => __('Créez le parc qui sera proposé à la réservation.'),
                'complete' => DB::table('vehicles')->where('tenant_id', $tenantId)->whereNull('deleted_at')->exists(),
                'url' => route('vehicles.create'),
                'action' => __('Ajouter un véhicule'),
            ],
            [
                'label' => __('Définir une règle tarifaire'),
                'description' => __('Configurez un tarif actif avant de prendre une réservation.'),
                'complete' => DB::table('pricing_rules')->where('tenant_id', $tenantId)->where('is_active', true)->exists(),
                'url' => route('pricing-rules.create'),
                'action' => __('Créer un tarif'),
            ],
            [
                'label' => __('Créer le premier client'),
                'description' => __('Enregistrez les coordonnées nécessaires à une location.'),
                'complete' => DB::table('customers')->where('tenant_id', $tenantId)->whereNull('deleted_at')->exists(),
                'url' => route('customers.create'),
                'action' => __('Ajouter un client'),
            ],
            [
                'label' => __('Créer la première réservation'),
                'description' => __('Validez le parcours métier avec une réservation réelle ou de démonstration.'),
                'complete' => DB::table('reservations')->where('tenant_id', $tenantId)->whereNull('deleted_at')->exists(),
                'url' => route('reservations.create'),
                'action' => __('Créer une réservation'),
            ],
        ]);
