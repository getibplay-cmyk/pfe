...
class CustomerPortalController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('portal.home', [
            ...
            'contracts' => $customer->rentalContracts()->where('agency_id', $customer->agency_id)->whereIn('status', ['ready', 'accepted', 'active', 'return_pending', 'returned', 'closed'])->with('currentVersion')->latest('expected_start_at')->paginate(10, ['*'], 'contracts_page'),
    ...
    public function upload(Request $request, StorePrivateDocument $store)
    {
        ...
        DB::transaction(function () use ($request, $store, $customer, $access, $data) {
            ...
            if (DB::table('customer_portal_uploads')->where('tenant_id', $customer->tenant_id)->where('customer_id', $customer->id)->where('created_at', '>=', now()->subDay())->count() >= 10) {
                throw ValidationException::withMessages(['file' => __('Vous avez atteint la limite de 10 documents par jour.')]);
            }
            $document = $store->handle($customer, ['document_type' => $data['document_type'],
                'title' => $data['document_type'] === 'customer_identity' ? __('Identité transmise par le locataire') : __('Justificatif transmis par le locataire'), 'is_sensitive' => true], $request->file('file'), null);
        ...
        return redirect()->route('portal.home')->with('status', __('Document transmis. Votre agence doit encore le vérifier.'));
    }
