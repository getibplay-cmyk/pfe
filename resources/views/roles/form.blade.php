                ...
                <x-section-card title="État du rôle" description="La désactivation retire immédiatement les capacités. Si le rôle est attribué, un remplacement contrôlé est obligatoire.">
                    ...
                    @if($replacementImpact['user_count'] > 0)
                        <div id="replacement-impact" class="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">
                            <p class="font-semibold">{{ $replacementImpact['user_count'] }} utilisateur(s) seront réaffectés.</p>
                            <ul class="mt-2 list-disc space-y-1 pl-5">@foreach($replacementImpact['agencies'] as $agency)<li>{{ $agency['name'] }} : {{ $agency['user_count'] }} utilisateur(s)</li>@endforeach</ul>
                        </div>
                    @endif
                    ...
                    @if($replacementImpact['user_count'] > 0)
                        <label class="mt-4 flex items-start gap-3 rounded-lg border border-slate-200 p-3 text-sm"><input type="checkbox" name="confirm_replacement" value="1" class="mt-1 rounded border-slate-300" @checked(old('confirm_replacement'))><span>Je confirme la réaffectation des {{ $replacementImpact['user_count'] }} utilisateur(s) indiqués ci-dessus.</span></label>
