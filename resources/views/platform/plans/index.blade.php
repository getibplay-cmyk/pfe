<x-app-layout>
    <div class="rf-page">
        <x-page-header title="Plans SaaS" eyebrow="Administration de la plateforme" description="Définissez les offres commerciales de RentFleet sans activer de passerelle bancaire.">
            <x-slot:actions><a href="{{ route('platform.subscriptions.index') }}" class="rf-button-secondary">Voir les abonnements</a></x-slot:actions>
        </x-page-header>
        ...
        <x-section-card title="Créer un plan" description="Aucun tarif n’est préchargé : chaque montant est défini explicitement par l’administrateur de la plateforme.">
            <form method="POST" action="{{ route('platform.plans.store') }}" class="grid gap-4 lg:grid-cols-2">
                @csrf
                ...
                <fieldset class="lg:col-span-2" x-data="{ features: @js(array_values(old('features', ['Accès à RentFleet']))) }">
                    <legend class="text-sm font-medium text-slate-700">Fonctionnalités descriptives</legend>
                    <div class="mt-2 space-y-2"><template x-for="(feature, index) in features" :key="index"><div class="flex gap-2"><input name="features[]" x-model="features[index]" required maxlength="160" class="w-full" :aria-label="`Fonctionnalité ${index + 1}`"><button type="button" class="rf-button-secondary" x-on:click="features.splice(index, 1)" x-bind:disabled="features.length === 1">Retirer</button></div></template></div>
                    <button type="button" class="rf-button-link mt-2" x-on:click="features.push('')" x-bind:disabled="features.length >= 30">Ajouter une fonctionnalité</button>
                    <noscript><input name="features[]" value="Accès à RentFleet" required class="mt-2 w-full"></noscript>
                ...
                <div class="lg:col-span-2"><x-primary-button>Créer le plan</x-primary-button></div>
            </form>
        </x-section-card>
        ...
        <x-filter-panel>
            <form class="rf-filter-grid" method="GET">
                ...
                <div class="flex items-end gap-2"><x-primary-button>Filtrer</x-primary-button><a href="{{ route('platform.plans.index') }}" class="rf-button-secondary">Effacer</a></div>
            </form>
        </x-filter-panel>
                ...
                <tbody>
                    @forelse($plans as $plan)
                        <tr>
                            ...
                            <td>{{ $plan->subscriptions_count }} abonnement(s)</td>
                            <td><x-status-badge :value="$plan->is_active ? 'active' : 'inactive'" /></td>
                            <td class="text-right">
                                <details class="inline-block text-left"><summary class="rf-button-secondary cursor-pointer">Modifier</summary>
                                    <form method="POST" action="{{ route('platform.plans.update', $plan) }}" class="mt-3 w-[min(34rem,80vw)] space-y-3 rounded-xl border border-slate-200 bg-white p-4 shadow-xl" x-data="{ features: @js(array_values($plan->features ?: ['Accès à RentFleet'])) }">
                                        @csrf @method('PATCH')
                                        ...
                                        <div class="space-y-2"><template x-for="(feature, index) in features" :key="index"><div class="flex gap-2"><input name="features[]" x-model="features[index]" required maxlength="160" class="w-full" :aria-label="`Fonctionnalité ${index + 1}`"><button type="button" class="rf-button-secondary" x-on:click="features.splice(index, 1)" x-bind:disabled="features.length === 1">Retirer</button></div></template><button type="button" class="rf-button-link" x-on:click="features.push('')">Ajouter</button></div>
