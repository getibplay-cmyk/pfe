<x-app-layout>
    ...
    <div class="rf-page">
        ...
        <x-section-card title="Créer un plan" description="Aucun tarif n’est préchargé : chaque montant est défini explicitement par l’administrateur de la plateforme.">
            <form method="POST" action="{{ route('platform.plans.store') }}" class="grid gap-4 lg:grid-cols-2" data-loading-form>
                ...
                <fieldset class="lg:col-span-2 rounded-xl border border-slate-200 p-4">
                    ...
                    <legend class="px-2 text-sm font-semibold text-slate-800">Droits et quotas contractuels</legend>
                    <p class="mb-4 text-sm text-slate-600">Laissez un quota vide pour un accès sans limite. La valeur est figée dans chaque nouvel abonnement.</p>
                    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                        <div><x-input-label for="plan-max-agencies" value="Agences actives" /><input id="plan-max-agencies" name="max_agencies" type="number" min="0" max="10000" value="{{ old('max_agencies') }}" class="mt-1 w-full" placeholder="Sans limite"><x-field-error :messages="$errors->get('max_agencies')" /></div>
                        <div><x-input-label for="plan-max-users" value="Utilisateurs actifs" /><input id="plan-max-users" name="max_users" type="number" min="0" max="100000" value="{{ old('max_users') }}" class="mt-1 w-full" placeholder="Sans limite"><x-field-error :messages="$errors->get('max_users')" /></div>
                        <div><x-input-label for="plan-max-vehicles" value="Véhicules" /><input id="plan-max-vehicles" name="max_vehicles" type="number" min="0" max="1000000" value="{{ old('max_vehicles') }}" class="mt-1 w-full" placeholder="Sans limite"><x-field-error :messages="$errors->get('max_vehicles')" /></div>
                        <div><x-input-label for="plan-max-intelligence" value="Analyses / mois" /><input id="plan-max-intelligence" name="monthly_intelligence_runs" type="number" min="0" max="10000000" value="{{ old('monthly_intelligence_runs') }}" class="mt-1 w-full" placeholder="Sans limite"><x-field-error :messages="$errors->get('monthly_intelligence_runs')" /></div>
                    </div>
        ...
        <x-responsive-table label="Plans SaaS">
            <table>
                <thead><tr><th>Plan</th><th>Périodicité</th><th>Prix</th><th>Quotas</th><th>Utilisation</th><th>État</th><th class="text-right">Gestion</th></tr></thead>
                <tbody>
                    @forelse($plans as $plan)
                        <tr>
                            ...
                            <td class="text-xs leading-5 text-slate-600">{{ $plan->entitlements['max_agencies'] ?? '∞' }} agences · {{ $plan->entitlements['max_users'] ?? '∞' }} utilisateurs<br>{{ $plan->entitlements['max_vehicles'] ?? '∞' }} véhicules · {{ $plan->entitlements['monthly_intelligence_runs'] ?? '∞' }} analyses/mois</td>
                                ...
                                <details class="inline-block text-left"><summary class="group relative inline-flex h-11 w-11 cursor-pointer list-none items-center justify-center rounded-xl border border-belkhir-space-border bg-white text-belkhir-space-muted shadow-sm transition hover:border-slate-400 hover:text-belkhir-space-blue [&::-webkit-details-marker]:hidden" aria-label="Modifier {{ $plan->name }}" title="Modifier {{ $plan->name }}"><x-icon name="edit" /><span class="sr-only">Modifier {{ $plan->name }}</span><span role="tooltip" class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 hidden -translate-x-1/2 whitespace-nowrap rounded-lg bg-belkhir-space-ink px-2.5 py-1.5 text-xs font-semibold text-white shadow-lg group-hover:block group-focus:block">Modifier {{ $plan->name }}</span></summary>
                                    <form method="POST" action="{{ route('platform.plans.update', $plan) }}" class="mt-3 w-[min(34rem,80vw)] space-y-3 rounded-xl border border-slate-200 bg-white p-4 shadow-xl" x-data="{ features: @js(array_values($plan->features ?: ['Accès à '.config('brand.name')])) }" data-loading-form>
                                        ...
                                        <fieldset class="rounded-xl border border-slate-200 p-3">
                                            <input type="hidden" name="entitlements_configured" value="1">
                                            <legend class="px-1 text-sm font-semibold">Droits et quotas</legend>
                                            <div class="grid gap-3 sm:grid-cols-2">
                                                <label class="text-xs text-slate-600">Agences actives<input name="max_agencies" type="number" min="0" max="10000" value="{{ $plan->entitlements['max_agencies'] }}" class="mt-1 w-full" placeholder="Sans limite"></label>
                                                <label class="text-xs text-slate-600">Utilisateurs actifs<input name="max_users" type="number" min="0" max="100000" value="{{ $plan->entitlements['max_users'] }}" class="mt-1 w-full" placeholder="Sans limite"></label>
                                                <label class="text-xs text-slate-600">Véhicules<input name="max_vehicles" type="number" min="0" max="1000000" value="{{ $plan->entitlements['max_vehicles'] }}" class="mt-1 w-full" placeholder="Sans limite"></label>
                                                <label class="text-xs text-slate-600">Analyses / mois<input name="monthly_intelligence_runs" type="number" min="0" max="10000000" value="{{ $plan->entitlements['monthly_intelligence_runs'] }}" class="mt-1 w-full" placeholder="Sans limite"></label>
                                            </div>
                    ...
                    @empty<tr><td colspan="7"><x-empty-state title="Aucun plan SaaS" description="Créez un premier plan lorsque ses conditions commerciales sont décidées." /></td></tr>@endforelse
                </tbody>
            </table>
