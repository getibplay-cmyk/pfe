<x-app-layout>
    ...
    <div class="rf-page" data-platform-statistics>
        <x-page-header
            :title="__('Statistiques de la plateforme')"
            :eyebrow="__('Administration SaaS')"
            :description="__('Une lecture consolidée de l’activité ').config('brand.name').__(', fondée exclusivement sur les données de la période sélectionnée.')"
        />
        ...
        <section class="rf-panel overflow-hidden" aria-labelledby="platform-statistics-filters-title">
            ...
            >
                <div>
                    <x-input-label for="platform-statistics-date-from" :value="__('Du')" />
                ...
                <div>
                    <x-input-label for="platform-statistics-date-to" :value="__('Au')" />
                ...
                <button type="submit" class="rf-button-primary min-h-11" x-bind:disabled="submitting" x-bind:aria-busy="submitting.toString()">
                    ...
                    <span x-text="submitting ? 'Actualisation…' : 'Actualiser'">{{ __('Actualiser') }}</span>
                </button>
                <a href="{{ route('platform.statistics.index') }}" class="rf-button-quiet min-h-11"><x-icon name="reset" size="xs" />{{ __('Réinitialiser') }}</a>
            </form>
        </section>

        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <x-stat-card :label="__('Entreprises clientes')" :value="App\Support\Ui\BusinessNumber::integer($tenantTotal)" icon="building" hint="Périmètre total de la plateforme" />
            <x-stat-card
                :label="__('Entreprises actives')"
                :value="App\Support\Ui\BusinessNumber::integer($statistics['totals']['active_tenants'])"
                icon="success"
                tone="success"
                :hint="$tenantTotal > 0 ? App\Support\Ui\BusinessNumber::integer($statistics['totals']['active_tenants']).' sur '.App\Support\Ui\BusinessNumber::integer($tenantTotal) : __('Aucune entreprise enregistrée')"
            />
            <x-stat-card
                :label="__('Paiements enregistrés')"
                :value="App\Support\Ui\BusinessNumber::integer($statistics['totals']['recorded_saas_payments'])"
                icon="payment"
                :hint="App\Support\Ui\BusinessNumber::count($statistics['payments']['reversal_count'], 'contrepassation').__(' sur la période')"
            />
            <x-stat-card
                :label="__('Traitements en échec')"
                ...
                :hint="__('Opérations en attente : ').App\Support\Ui\BusinessNumber::integer($statistics['totals']['jobs'])"
            />
        </div>
        ...
        <section class="grid gap-4 rounded-2xl bg-belkhir-space-ink p-5 text-white shadow-lg shadow-belkhir-space-ink/10 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)] lg:items-center">
            ...
            @else
                <p class="rounded-xl border border-white/15 bg-white/5 p-4 text-sm text-slate-300">{{ __('Aucune entreprise n’est enregistrée : aucun taux n’est affiché.') }}</p>
            @endif
        </section>
        ...
        <div class="grid gap-6 xl:grid-cols-2">
            <x-section-card :title="__('Entreprises par état')" :description="__('Répartition actuelle des entreprises clientes.')">
                <div class="rf-chart-surface h-80 overflow-hidden" data-chart-shell data-chart-ready="false" aria-busy="true">
                    ...
                    <canvas class="opacity-0 transition-opacity duration-500 motion-reduce:transition-none" role="img" data-platform-chart="tenant-states" aria-label="{{ __('Répartition des entreprises clientes par état') }}" aria-describedby="platform-tenant-states-table"></canvas>
                </div>
                <div id="platform-tenant-states-table" class="mt-5">
                    <table class="w-full text-sm"><caption class="sr-only">{{ __('Répartition des entreprises clientes par état') }}</caption><tbody>@foreach($statistics['tenant_states'] as $state)<tr class="border-t border-slate-100"><th scope="row" class="py-2.5 text-start font-medium text-slate-700">{{ $state['label'] }}</th><td class="py-2.5 text-end font-semibold text-slate-950">{{ App\Support\Ui\BusinessNumber::integer($state['total']) }}</td></tr>@endforeach</tbody></table>
                </div>
            </x-section-card>

            <x-section-card :title="__('Abonnements par état')" :description="__('Répartition actuelle, tous plans confondus.')">
                <div class="rf-chart-surface h-80 overflow-hidden" data-chart-shell data-chart-ready="false" aria-busy="true">
                    <x-skeleton variant="chart" :label="__('Chargement du graphique des abonnements…')" class="absolute inset-3 z-10 motion-reduce:animate-none" data-chart-skeleton />
                    <canvas class="opacity-0 transition-opacity duration-500 motion-reduce:transition-none" role="img" data-platform-chart="subscription-states" aria-label="{{ __('Répartition des abonnements par état') }}" aria-describedby="platform-subscription-states-table"></canvas>
                </div>
                <div id="platform-subscription-states-table" class="mt-5">
                    <div class="mb-3 flex items-center justify-between text-sm"><span class="text-slate-500">{{ __('Abonnements enregistrés') }}</span><strong>{{ App\Support\Ui\BusinessNumber::integer($subscriptionTotal) }}</strong></div>
                    <table class="w-full text-sm"><caption class="sr-only">{{ __('Répartition des abonnements par état') }}</caption><tbody>@foreach($statistics['subscription_states'] as $state)<tr class="border-t border-slate-100"><th scope="row" class="py-2.5 text-start font-medium text-slate-700">{{ $state['label'] }}</th><td class="py-2.5 text-end font-semibold text-slate-950">{{ App\Support\Ui\BusinessNumber::integer($state['total']) }}</td></tr>@endforeach</tbody></table>
                </div>
            </x-section-card>

            <x-section-card :title="__('Analyses mensuelles')" :description="__('Nombre d’analyses consultatives demandées pendant la période.')">
                <div class="rf-chart-surface h-80 overflow-hidden" data-chart-shell data-chart-ready="false" aria-busy="true">
                    <x-skeleton variant="chart" :label="__('Chargement du graphique des analyses…')" class="absolute inset-3 z-10 motion-reduce:animate-none" data-chart-skeleton />
                    <canvas class="opacity-0 transition-opacity duration-500 motion-reduce:transition-none" role="img" data-platform-chart="activity" aria-label="{{ __('Évolution mensuelle du nombre d’analyses') }}" aria-describedby="platform-activity-table"></canvas>
                </div>
                <table id="platform-activity-table" class="mt-5 w-full text-sm"><caption class="sr-only">{{ __('Analyses mensuelles') }}</caption><thead><tr class="border-b border-slate-200"><th scope="col" class="py-2 text-start">{{ __('Mois') }}</th><th scope="col" class="py-2 text-end">{{ __('Analyses') }}</th></tr></thead><tbody>@foreach($statistics['monthly_runs'] as $month)<tr class="border-t border-slate-100"><th scope="row" class="py-2.5 text-start font-medium text-slate-700">{{ $month['label'] }}</th><td class="py-2.5 text-end font-semibold text-slate-950">{{ App\Support\Ui\BusinessNumber::integer($month['total']) }}</td></tr>@endforeach</tbody></table>
            </x-section-card>

            <x-section-card :title="__('Activations par capacité')" :description="__('Nombre d’entreprises autorisées, comparé au total réel d’entreprises clientes.')">
                <div class="rf-chart-surface h-80 overflow-hidden" data-chart-shell data-chart-ready="false" aria-busy="true">
                    <x-skeleton variant="chart" :label="__('Chargement du graphique des activations…')" class="absolute inset-3 z-10 motion-reduce:animate-none" data-chart-skeleton />
                    <canvas class="opacity-0 transition-opacity duration-500 motion-reduce:transition-none" role="img" data-platform-chart="activations" aria-label="{{ __('Nombre d’entreprises autorisées pour chaque capacité') }}" aria-describedby="platform-activations-table"></canvas>
                </div>
                <div id="platform-activations-table" class="mt-5 overflow-x-auto">
                    <table class="w-full min-w-[32rem] text-sm">
                        <caption class="sr-only">{{ __('Entreprises autorisées par capacité') }}</caption>
                        <thead><tr class="border-b border-slate-200"><th scope="col" class="py-2 text-start">{{ __('Capacité') }}</th><th scope="col" class="py-2 text-end">{{ __('Entreprises') }}</th><th scope="col" class="w-1/2 py-2 ps-5 text-start">{{ __('Couverture') }}</th></tr></thead>
                        <tbody>
                            @foreach ($statistics['activations'] as $activation)
                                <tr class="border-t border-slate-100">
                                    <th scope="row" class="py-3 text-start font-medium text-slate-700">{{ $activation['label'] }}</th>
                                    <td class="py-3 text-end font-semibold">{{ App\Support\Ui\BusinessNumber::integer($activation['tenant_count']) }} / {{ App\Support\Ui\BusinessNumber::integer($tenantTotal) }}</td>
                                    <td class="py-3 ps-5">
                                        @if ($tenantTotal > 0)
                                            <x-progress-bar
                                                :label="__('Couverture de ').$activation['label']"
                                            ...
                                            />
                                        @else
                                            <span class="text-slate-500">{{ __('Non calculable sans entreprise') }}</span>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
        ...
        <div class="grid gap-6 xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
            <x-section-card :title="__('Encaissements SaaS')" :description="__('Paiements manuels et CMI, nets après contrepassations et toujours séparés par devise.')">
                ...
                @empty
                    <x-empty-state :title="__('Aucun paiement sur cette période')" :description="__('Aucun montant n’est représenté sans donnée enregistrée.')" />
                @endforelse
            </x-section-card>

            <x-responsive-table :label="__('Analyses par capacité et par état')">
                <table class="rf-table"><thead><tr><th>{{ __('Assistance') }}</th><th>{{ __('État') }}</th><th class="text-end">{{ __('Nombre') }}</th></tr></thead><tbody>
                    @forelse($statistics['run_states'] as $run)<tr><td>{{ $run['label'] }}</td><td><x-status-badge :value="$run['status']" /></td><td class="text-end font-semibold">{{ $run['total'] }}</td></tr>
                    @empty<tr><td colspan="3"><x-empty-state :title="__('Aucune analyse sur cette période')" /></td></tr>@endforelse
                </tbody></table>
            </x-responsive-table>
        </div>
