...
<x-app-layout>
    <div class="rf-page max-w-6xl">
        <x-page-header
            ...
            :eyebrow="__('Accès')"
            :description="__('Comptes limités à votre entreprise et, pour les responsables d’agence, à leur agence.')"
        >
            <x-slot:actions>
        ...
        <x-filter-panel id="user-filters" :title="__('Rechercher un utilisateur')" :active-count="$activeFilters" :result-count="$users->total()">
        ...
        <x-responsive-table :label="__('Utilisateurs')">
            <table>
                <thead><tr><th>{{ __('Utilisateur') }}</th><th>{{ __('Rôle') }}</th><th>{{ __('Agence') }}</th><th>{{ __('Statut') }}</th><th>{{ __('Dernière activité') }}</th><th class="text-end"><span class="sr-only">{{ __('Actions') }}</span></th></tr></thead>
                <tbody>
                        ...
                        <tr>
                            ...
                            <td><div class="flex justify-end">
                                @can('update', $managedUser)
                                    <x-icon-button icon="edit" :label="__('Modifier l’utilisateur ').$managedUser->name" :href="route('users.edit', $managedUser)" />
                                @endcan
                            </div></td>
                        </tr>
                    @empty
                        <tr><td colspan="6" class="p-4"><x-empty-state :title="__('Aucun utilisateur trouvé')" :description="__('Aucun utilisateur ne correspond aux filtres sélectionnés.')" /></td></tr>
                    @endforelse
                </tbody>
            </table>
        </x-responsive-table>
