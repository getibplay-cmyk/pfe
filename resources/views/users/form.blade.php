        ...
        <form class="space-y-5" method="POST" action="{{ $managedUser->exists ? route('users.update', $managedUser) : route('users.store') }}">
            ...
            <x-section-card title="Compte utilisateur">
                <div class="grid gap-5 sm:grid-cols-2">
                    <div><x-input-label for="user-name" value="Nom" required /><input id="user-name" class="mt-1 w-full" name="name" value="{{ old('name', $managedUser->name) }}" aria-describedby="user-name-error" required><x-field-error id="user-name-error" :messages="$errors->get('name')" /></div>
                    <div><x-input-label for="user-email" value="E-mail" required /><input id="user-email" type="email" class="mt-1 w-full" name="email" value="{{ old('email', $managedUser->email) }}" aria-describedby="user-email-error" required><x-field-error id="user-email-error" :messages="$errors->get('email')" /></div>
                    <div><x-input-label for="user-role" value="Rôle autorisé" required /><select id="user-role" class="mt-1 w-full" name="role_id" aria-describedby="user-role-help user-role-error" required>@foreach($roles as $role)<option value="{{ $role->id }}" @selected(old('role_id', $managedUser->role_id) == $role->id)>{{ $role->displayName() }}</option>@endforeach</select><p id="user-role-help" class="mt-1 text-xs text-slate-500">La liste est filtrée côté serveur selon vos délégations et votre plafond de permissions.</p><x-field-error id="user-role-error" :messages="$errors->get('role_id')" /></div>
                    <div><x-input-label for="user-agency" value="Agence" /><select id="user-agency" class="mt-1 w-full" name="agency_id" aria-describedby="user-agency-help user-agency-error"><option value="">Toutes les agences — administrateur uniquement</option>@foreach($agencies as $agency)<option value="{{ $agency->id }}" @selected(old('agency_id', $managedUser->agency_id) == $agency->id)>{{ $agency->name }}</option>@endforeach</select><p id="user-agency-help" class="mt-1 text-xs text-slate-500">Un responsable d’agence reste limité à sa propre agence.</p><x-field-error id="user-agency-error" :messages="$errors->get('agency_id')" /></div>
                </div>
