...
@if($billingState !== null && !$billingState['legacy'])
    ...
    @if(!$billingState['available'] || ($deadline && $deadline->lte(now()->addDays(7))))
        <x-section-card title="Votre abonnement demande votre attention">
            <p class="text-sm text-slate-700">{{ $billingState['reason'] }} @if($deadline) Échéance : {{ App\Support\Ui\UiLabel::dateTime($deadline) }}.@endif</p>
            <a href="{{ route('tenant-saas-account.show') }}" class="rf-button-primary mt-3">Gérer l’abonnement et les factures</a>
        </x-section-card>
    @endif
@endif
@if(count($groups))
    <section aria-labelledby="actions-today" data-dashboard-actions>
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h2 id="actions-today" class="text-xl font-bold text-slate-900">À traiter aujourd’hui</h2><p class="text-sm text-slate-500">Les cinq premières priorités de chaque catégorie, dans votre périmètre.</p></div>@if(auth()->user()->hasPermission('vehicle.view'))<a class="rf-button-secondary" href="{{ route('fleet.planning.index') }}">Ouvrir le planning</a>@endif</div>
