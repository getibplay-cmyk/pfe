            ...
            @foreach ($items as $item)
                <li class="flex min-w-0 items-center gap-2">
                    ...
                    @if ($loop->last || empty($item['url']))
                        <span class="truncate font-medium text-belkhir-space-text" @if($loop->last) aria-current="page" @endif>{{ $item['label'] }}</span>
                    @else
                        <a class="truncate font-medium text-belkhir-space-blue hover:text-belkhir-space-blue-hover hover:underline" href="{{ $item['url'] }}">{{ $item['label'] }}</a>
                    @endif
                </li>
            @endforeach
        </ol>
