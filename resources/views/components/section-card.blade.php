...
<{{ $as }} {{ $attributes->class('rf-panel overflow-hidden') }}>
    @if ($title || $description || isset($actions))
        <header class="rf-panel-heading flex flex-wrap items-start justify-between gap-3">
            <div>
                @if ($title)<h2 class="font-semibold text-belkhir-space-text">{{ $title }}</h2>@endif
                @if ($description)<p class="mt-1 text-sm leading-5 text-belkhir-space-muted">{{ $description }}</p>@endif
            </div>
