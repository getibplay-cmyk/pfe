<x-public-layout :title="__('Abonnement')">
    ...
    <section class="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
        <ol class="grid gap-5 md:grid-cols-3" aria-label="{{ __('Étapes d’abonnement') }}">
            @foreach ([
        ...
        <div class="mt-10 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            <article class="rf-panel p-6 sm:p-8"><h2 class="text-2xl font-bold text-slate-950">{{ __('Déjà client') }}</h2><p class="mt-3 text-sm leading-6 text-slate-600">{{ __('Votre administrateur d’entreprise peut consulter le plan, les échéances, les paiements et les fonctions activées depuis son espace SaaS.') }}</p><a href="{{ route('login') }}" class="rf-button-primary mt-6"><x-icon name="login" size="xs" />{{ __('Se connecter') }}</a></article>
            <article class="rounded-2xl border border-amber-200 bg-amber-50 p-6 sm:p-8"><h2 class="text-xl font-bold text-amber-950">{{ __('Nouvelle entreprise') }}</h2><p class="mt-3 text-sm leading-6 text-amber-900">{{ __('La création publique de comptes est volontairement désactivée. Écrivez à l’équipe commerciale en indiquant l’offre souhaitée') }}{{ request('plan') ? ' : '.request('plan') : '' }}.</p><a href="mailto:{{ config('brand.sales_email') }}" class="rf-button mt-6 border-amber-300 bg-white text-amber-950 hover:bg-amber-100"><x-icon name="mail" size="xs" />{{ __('Contacter') }} {{ config('brand.name') }}</a></article>
        </div>
        <div class="mt-10 rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-sm leading-6 text-emerald-950"><strong>{{ __('Paiement protégé :') }}</strong> {{ __('le numéro de carte, sa date d’expiration et son cryptogramme sont saisis uniquement chez CMI. Le serveur') }} {{ config('brand.name') }} {{ __('conserve une référence de transaction, le montant et le résultat signé nécessaires au suivi comptable.') }}</div>
    </section>
</x-public-layout>
