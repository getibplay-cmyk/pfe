<x-app-layout>
    <div class="mx-auto max-w-7xl space-y-6">
        ...
        <form class="grid gap-3 rounded-xl bg-white p-4 shadow-sm md:grid-cols-3">
            ...
            <button class="inline-flex items-center justify-center gap-2 rounded-lg bg-slate-800 px-4 py-2 text-white"><x-icon name="filter" size="xs" />Filtrer</button>
        </form>
        <div class="overflow-x-auto rounded-xl bg-white shadow-sm"><table class="min-w-full text-sm"><thead class="bg-slate-50 text-left"><tr><th class="p-3">Nom</th><th class="p-3">Portée</th><th class="p-3">Catégorie</th><th class="p-3">Journalier</th><th class="p-3">Validité</th><th class="p-3">État</th><th class="p-3"></th></tr></thead><tbody>
            @forelse($rules as $rule)<tr class="border-t"><td class="p-3 font-medium">{{ $rule->name }} <span class="text-xs text-slate-400">#{{ $rule->id }}</span></td><td class="p-3">{{ $rule->agency?->name ?? 'Toute l’entreprise' }}</td><td class="p-3">{{ $rule->vehicleCategory->name }}</td><td class="p-3">{{ App\Support\Ui\UiLabel::money($rule->daily_rate, $rule->currency) }}</td><td class="p-3">{{ $rule->valid_from->format('d/m/Y') }} – {{ $rule->valid_to?->format('d/m/Y') ?? 'sans fin' }}</td><td class="p-3">{{ $rule->is_active ? 'Active' : 'Archivée' }}</td><td class="p-3">@can('update', $rule)<a class="inline-flex items-center gap-2 text-indigo-700" href="{{ route('pricing-rules.edit', $rule) }}"><x-icon name="add" size="xs" />Créer une version</a>@endcan</td></tr>@empty<tr><td colspan="7" class="p-8 text-center text-slate-500">Aucune règle tarifaire.</td></tr>@endforelse
