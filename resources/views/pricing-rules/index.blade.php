<x-app-layout>
    <div class="mx-auto max-w-7xl space-y-6">
        ...
        <div class="overflow-x-auto rounded-xl bg-white shadow-sm"><table class="min-w-full text-sm"><thead class="bg-slate-50 text-left"><tr><th class="p-3">Nom</th><th class="p-3">Portée</th><th class="p-3">Catégorie</th><th class="p-3">Journalier</th><th class="p-3">Validité</th><th class="p-3">État</th><th class="p-3"></th></tr></thead><tbody>
            @forelse($rules as $rule)<tr class="border-t"><td class="p-3 font-medium">{{ $rule->name }} <span class="text-xs text-slate-400">#{{ $rule->id }}</span></td><td class="p-3">{{ $rule->agency?->name ?? 'Toute l’entreprise' }}</td><td class="p-3">{{ $rule->vehicleCategory->name }}</td><td class="p-3">{{ $rule->daily_rate }} {{ $rule->currency }}</td><td class="p-3">{{ $rule->valid_from->format('d/m/Y') }} – {{ $rule->valid_to?->format('d/m/Y') ?? 'sans fin' }}</td><td class="p-3">{{ $rule->is_active ? 'Active' : 'Archivée' }}</td><td class="p-3">@can('update', $rule)<a class="text-indigo-700" href="{{ route('pricing-rules.edit', $rule) }}">Créer une version</a>@endcan</td></tr>@empty<tr><td colspan="7" class="p-8 text-center text-slate-500">Aucune règle tarifaire.</td></tr>@endforelse
