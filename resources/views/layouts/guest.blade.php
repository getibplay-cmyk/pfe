...
<head>
    ...
    @php($authTitle = match (true) { request()->routeIs('login') => __('Connexion'), request()->routeIs('password.request') => __('Mot de passe oublié'), request()->routeIs('password.reset') => __('Réinitialiser le mot de passe'), request()->routeIs('password.confirm') => __('Confirmer le mot de passe'), request()->routeIs('password.change-required') => __('Choisir un mot de passe'), request()->routeIs('verification.notice') => __('Vérifier l’adresse e-mail'), request()->routeIs('onboarding-invitations.*') => __('Créer votre espace'), default => __('Accès sécurisé') })
...
<body class="bg-belkhir-space-canvas font-sans antialiased">
    ...
    <a href="#contenu" class="rf-skip-link">{{ __('Aller au formulaire') }}</a>
    <main id="contenu" class="flex min-h-screen min-h-[100dvh] flex-col lg:grid lg:grid-cols-[minmax(24rem,0.92fr)_minmax(30rem,1.08fr)]">
        <section class="relative flex min-h-[12.5rem] shrink-0 overflow-hidden bg-belkhir-space-ink px-5 py-6 text-white sm:px-8 lg:min-h-screen lg:flex-col lg:justify-between lg:p-12 xl:p-16" aria-label="{{ __('Présentation de ') }}{{ config('brand.name') }}">
            ...
            <div class="relative z-10 flex w-full flex-col justify-between gap-6 lg:h-full">
                ...
                <div class="max-w-xl lg:my-auto">
                    <p class="hidden border-s-4 border-belkhir-space-orange ps-3 text-xs font-bold uppercase tracking-[0.18em] text-brand-300 sm:block">{{ __('SaaS B2B multi-entreprises') }}</p>
                    <h2 class="mt-2 max-w-lg text-2xl font-bold leading-tight tracking-tight sm:text-3xl lg:mt-5 lg:text-4xl xl:text-5xl">{{ __('Pilotez votre activité de location en toute clarté.') }}</h2>
                    <p class="mt-5 hidden max-w-lg text-base leading-7 text-slate-300 lg:block">{{ config('brand.name') }} {{ __('rassemble les opérations essentielles de votre agence dans un espace sécurisé et adapté à chaque rôle.') }}</p>

                    <ul class="mt-8 hidden gap-3 lg:grid" aria-label="{{ __('Fonctions principales') }}">
                ...
                <p class="hidden text-xs leading-5 text-slate-400 lg:block">{{ __('Accès réservé aux organisations clientes et à leurs collaborateurs autorisés.') }}</p>
            </div>
        </section>
        ...
        <section class="flex flex-1 items-start justify-center bg-belkhir-space-canvas px-4 py-8 sm:px-8 sm:py-12 lg:items-center lg:px-12 xl:px-20">
            <div class="w-full max-w-lg">
                <div class="mb-4 flex justify-end"><x-locale-switcher /></div>
                ...
                <p class="mx-auto mt-5 max-w-md text-center text-xs leading-5 text-belkhir-space-muted">{{ request()->routeIs('onboarding-invitations.*') ? __('Création réservée aux destinataires d’une invitation personnelle valide.') : __('Pas encore de compte ? L’inscription publique est désactivée. Contactez l’administrateur de votre organisation.') }}</p>
            </div>
        </section>
    </main>
</body>
