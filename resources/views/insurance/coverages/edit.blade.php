<x-app-layout>
    <div class="mx-auto max-w-3xl space-y-6">
        <x-page-header :title="__('Modifier la garantie')" :eyebrow="__('Assurance')" :description="__('Mettez à jour les limites contractuelles dans la devise de la police.')">
            <x-slot:actions><a href="{{ route('insurance.policies.show', $policy) }}" class="rf-button-secondary"><x-icon name="previous" size="xs" />{{ __('Retour à la police') }}</a></x-slot:actions>
        </x-page-header>
        <x-form-errors />
        <form method="POST" action="{{ route('insurance.coverages.update', [$policy, $coverage]) }}" class="grid gap-5 rounded-xl bg-white p-6 shadow-sm md:grid-cols-2">
            ...
            @method('PUT')
            <div>
                <x-input-label for="coverage-edit-type" :value="__('Type de garantie')" required />
            ...
            <div>
                <x-input-label for="coverage-edit-label" :value="__('Libellé de la garantie')" required />
            ...
            <div>
                <x-input-label for="coverage-edit-limit" :value="__('Plafond (').$policy->currency.')'" />
            ...
            <div>
                <x-input-label for="coverage-edit-deductible" :value="__('Franchise (').$policy->currency.')'" />
            ...
            <div class="md:col-span-2 flex justify-end"><button class="rf-button-primary"><x-icon name="save" size="xs" />{{ __('Enregistrer') }}</button></div>
        </form>
    </div>
</x-app-layout>
