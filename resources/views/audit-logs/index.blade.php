<x-app-layout>
    <div class="mx-auto max-w-6xl space-y-6">
        ...
        <x-responsive-table :label="__('Journal d’audit')"><table><thead><tr><th>{{ __('Date') }}</th><th>{{ __('Action') }}</th><th>{{ __('Élément') }}</th><th>{{ __('Acteur') }}</th></tr></thead><tbody>
            @forelse($logs as $log)<tr><td>{{ App\Support\Ui\UiLabel::dateTime($log->created_at) }}</td><td class="font-medium">{{ App\Support\Ui\UiLabel::action($log->action) }}</td><td>{{ App\Support\Ui\UiLabel::entity($log->auditable_type) }} #{{ $log->auditable_id }}</td><td>{{ $log->user?->name ?? __('Système') }}</td></tr>
