<x-guest-layout>
    <x-auth-heading
        icon="mail"
        :eyebrow="__('Récupération du compte')"
        :title="__('Mot de passe oublié')"
        :description="__('Saisissez votre adresse e-mail professionnelle. Si un compte est associé, vous recevrez un lien de réinitialisation.')"
    />
    ...
    <form method="POST" action="{{ route('password.email') }}" class="mt-7 space-y-5" data-loading-form>
        ...
        <x-submit-button :label="__('Envoyer le lien de réinitialisation')" loading-label="{{ __('Envoi en cours…') }}" icon="mail" class="w-full" />
    </form>
