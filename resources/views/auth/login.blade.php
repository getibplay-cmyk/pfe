<x-guest-layout>
    <x-auth-heading
        :eyebrow="__('Espace sécurisé')"
        :title="__('Connexion à ').config('brand.name')"
        :description="__('Accédez à l’espace de travail de votre organisation avec le compte fourni par votre administrateur.')"
    />
    ...
    <form method="POST" action="{{ route('login') }}" class="mt-7 space-y-5" data-loading-form>
        ...
        <x-password-field id="password" name="password" :label="__('Mot de passe')" :messages="$errors->get('password')" autocomplete="current-password" />
        <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            @if (Route::has('password.request'))
                <a class="rf-button-link -me-3" href="{{ route('password.request') }}">{{ __('Mot de passe oublié ?') }}</a>
            @endif
        </div>
        <x-submit-button :label="__('Se connecter')" loading-label="{{ __('Connexion en cours…') }}" class="w-full" />
    </form>
    <div class="mt-6 flex gap-3 rounded-xl border border-belkhir-space-border bg-belkhir-space-canvas p-4 text-xs leading-5 text-belkhir-space-muted">
        ...
        <p><strong class="text-belkhir-space-text">{{ __('Accès professionnel :') }}</strong> {{ __('les comptes sont créés par un administrateur autorisé. Aucun compte ne peut être ouvert publiquement.') }}</p>
    </div>
</x-guest-layout>
