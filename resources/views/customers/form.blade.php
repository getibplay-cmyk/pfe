<x-app-layout>
    <form data-unsaved-warning class="mx-auto max-w-4xl space-y-5 rounded-xl border border-slate-200 bg-white p-6 shadow-sm" method="POST" action="{{ $customer->exists ? route('customers.update', $customer) : route('customers.store') }}">
        ...
        <h1 class="text-2xl font-bold">{{ $customer->exists ? __('Modifier le client') : __('Nouveau client') }}</h1>
        <x-form-errors />

        @if ($customer->exists)
            <p class="text-sm text-slate-600">{{ __('Statut de vérification :') }} <x-status-badge :value="$customer->verification_status" /></p>
        @endif
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            ...
            <label class="text-sm">{{ __('Agence *') }}
                <select name="agency_id" required class="mt-1 w-full">
                    <option value="" disabled @selected(! old('agency_id', $customer->agency_id))>{{ __('Choisir une agence') }}</option>
            ...
            <label class="text-sm sm:col-span-2">{{ __('Adresse') }}
                <textarea class="mt-1 w-full" name="address" rows="2">{{ old('address', $customer->address) }}</textarea>
                <x-input-error :messages="$errors->get('address')" />
            </label>
            <label class="text-sm sm:col-span-2">{{ __('Notes') }}
                <textarea class="mt-1 w-full" name="notes" rows="3">{{ old('notes', $customer->notes) }}</textarea>
                <x-input-error :messages="$errors->get('notes')" />
            </label>
        </div>
        <p class="text-xs text-slate-500">{{ __('Le statut est géré uniquement par les actions Vérifier ou Rejeter. Les numéros d’identité restent chiffrés et masqués.') }}</p>
        <button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-slate-950 px-4 py-2 text-white"><x-icon name="save" size="xs" />{{ __('Enregistrer') }}</button>
    </form>
</x-app-layout>
