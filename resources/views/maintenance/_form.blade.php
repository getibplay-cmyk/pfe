...
<label class="text-sm md:col-span-2">{{ __('Description') }}
...
<label class="text-sm">{{ __('Début planifié') }}
...
<label class="text-sm">{{ __('Fin planifiée') }}
...
<label class="text-sm">{{ __('Kilométrage d’ouverture') }}
...
<label class="text-sm">{{ __('Coût estimé (MAD)') }}
...
<label class="text-sm md:col-span-2">{{ __('Prestataire') }}
...
<div class="md:col-span-2"><button class="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-white"><x-icon :name="$editing ? 'save' : 'add'" size="xs" />{{ $editing ? __('Enregistrer les modifications') : __('Créer l’ordre') }}</button></div>
