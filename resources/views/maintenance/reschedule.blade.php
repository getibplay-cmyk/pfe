<x-app-layout>
    <div class="mx-auto max-w-3xl space-y-6">
        ...
        <form method="POST" action="{{ route('maintenance.reschedule', $maintenance) }}" class="grid gap-5 rounded-xl bg-white p-6 shadow-sm sm:grid-cols-2" x-belkhir-space-confirm data-confirm-title="Replanifier la maintenance" data-confirm-resource="Ordre de maintenance sélectionné" data-confirm-consequence="La nouvelle période remplacera la période planifiée selon les contrôles de disponibilité existants." data-confirm-label="Replanifier">
            @csrf @method('PATCH')
