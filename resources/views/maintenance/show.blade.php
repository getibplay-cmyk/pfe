<x-app-layout>
    <div class="mx-auto max-w-7xl space-y-6">
        ...
        <section class="rounded-xl bg-white p-5 shadow-sm"><h2 class="font-semibold">Actions autorisées</h2><div class="mt-4 space-y-4">
            @can('approve', $maintenance)<form method="POST" action="{{ route('maintenance.approve', $maintenance) }}" x-belkhir-space-confirm data-confirm-title="Approuver la maintenance" data-confirm-resource="Ordre de maintenance sélectionné" data-confirm-consequence="L’ordre sera approuvé et le véhicule sera bloqué selon la période planifiée." data-confirm-label="Approuver">@csrf<button class="rounded-lg bg-indigo-700 px-4 py-2 text-white">Approuver</button></form>@endcan
            @can('start', $maintenance)<form method="POST" action="{{ route('maintenance.start', $maintenance) }}" x-belkhir-space-confirm data-confirm-title="Démarrer la maintenance" data-confirm-resource="Ordre de maintenance sélectionné" data-confirm-consequence="La maintenance passera à l’état en cours selon les règles existantes." data-confirm-label="Démarrer">@csrf<button class="rounded-lg bg-indigo-700 px-4 py-2 text-white">Démarrer</button></form>@endcan
            @can('complete', $maintenance)
                <form method="POST" action="{{ route('maintenance.complete', $maintenance) }}" class="grid gap-3 md:grid-cols-3" x-belkhir-space-confirm data-confirm-title="Terminer la maintenance" data-confirm-resource="Ordre de maintenance sélectionné" data-confirm-consequence="Les informations finales seront enregistrées et cette transition ne pourra pas être annulée depuis cet écran." data-confirm-label="Terminer">@csrf
        ...
        <section class="rounded-xl bg-white p-5 shadow-sm">
            ...
            @can('uploadDocument', $maintenance)
                <form method="POST" enctype="multipart/form-data" action="{{ route('maintenance.documents.store', $maintenance) }}" class="mt-5 grid gap-3 border-t pt-5 sm:grid-cols-2" data-loading-form>@csrf<input type="hidden" name="is_sensitive" value="1">
                    ...
                    <x-file-input id="maintenance-document-file" name="file" label="Fichier privé" required :errors="$errors->get('file')" />
                    ...
                    <div class="sm:col-span-2"><x-submit-button label="Ajouter le document privé" loading-label="Ajout en cours…" /></div>
                </form>
            @endcan
        </section>
    </div>
