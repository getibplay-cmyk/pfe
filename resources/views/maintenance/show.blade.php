<x-app-layout>
    <div class="mx-auto max-w-6xl space-y-6">
        <div class="flex flex-wrap items-start justify-between gap-3">
            ...
            <x-status-badge :value="$maintenance->status" />
        </div>
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            <section class="rounded-xl bg-white p-5 shadow-sm lg:col-span-2">
                ...
                <dl class="mt-4 grid gap-3 text-sm sm:grid-cols-2"><div><dt class="text-slate-500">Véhicule</dt><dd><a class="underline" href="{{ route('vehicles.show', $maintenance->vehicle) }}">{{ $maintenance->vehicle->registration_number }}</a></dd></div><div><dt class="text-slate-500">Type</dt><dd>{{ App\Support\Ui\UiLabel::get($maintenance->maintenance_type) }}</dd></div><div><dt class="text-slate-500">Période</dt><dd>{{ App\Support\Ui\UiLabel::dateTime($maintenance->scheduled_start_at) }} → {{ App\Support\Ui\UiLabel::dateTime($maintenance->scheduled_end_at) }}</dd></div><div><dt class="text-slate-500">Coûts</dt><dd>{{ App\Support\Ui\UiLabel::money($maintenance->estimated_cost, 'MAD') }} / {{ App\Support\Ui\UiLabel::money($maintenance->actual_cost, 'MAD') }}</dd></div></dl>
            </section>
            <section class="rounded-xl bg-white p-5 shadow-sm"><h2 class="font-semibold">Bloc véhicule</h2>@if($maintenance->vehicleBlock)<p class="mt-3 text-sm">{{ App\Support\Ui\UiLabel::get($maintenance->vehicleBlock->status) }} · {{ App\Support\Ui\UiLabel::dateTime($maintenance->vehicleBlock->starts_at) }} → {{ App\Support\Ui\UiLabel::dateTime($maintenance->vehicleBlock->ends_at) }}</p>@else<p class="mt-3 text-sm text-slate-500">Aucun bloc avant approbation.</p>@endif</section>
        </div>
