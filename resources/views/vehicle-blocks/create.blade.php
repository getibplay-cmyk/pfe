<x-app-layout>
    <div class="mx-auto max-w-3xl space-y-6">
        <x-page-header :title="__('Nouveau bloc manuel')" :eyebrow="__('Flotte')" :description="__('La période sera protégée par la contrainte anti-chevauchement PostgreSQL.')" />
        <x-form-errors />
        ...
        <form method="POST" action="{{ route('vehicle-blocks.store') }}" class="grid gap-4 rounded-xl bg-white p-6 shadow-sm sm:grid-cols-2">
            ...
            <label class="text-sm">{{ __('Véhicule actif *') }}
            ...
            <label class="text-sm sm:col-span-2">{{ __('Motif *') }}
                <textarea name="reason" required maxlength="2000" rows="4" class="mt-1 w-full">{{ old('reason') }}</textarea>
                <x-input-error :messages="$errors->get('reason')" />
            </label>
            <div class="flex gap-3 sm:col-span-2">
                <button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-white"><x-icon name="add" size="xs" />{{ __('Créer le bloc') }}</button>
                <a href="{{ route('vehicle-blocks.index') }}" class="rounded-lg border px-4 py-2">{{ __('Annuler') }}</a>
            </div>
        </form>
    </div>
</x-app-layout>
