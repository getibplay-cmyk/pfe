    ...
    <div class="rf-page">
            ...
            <section class="space-y-5" data-tenant-statistics>
                ...
                <div class="rf-panel p-5">
                    @if ($dashboardStatistics['availability']['total'] > 0)
                        ...
                        />
                    @else
                        <x-empty-state :title="__('Aucun véhicule dans ce périmètre')" :description="__('La disponibilité apparaîtra après l’ajout d’un véhicule autorisé.')" />
                    @endif
                </div>
                <div class="grid gap-6 xl:grid-cols-2">
                    <x-tenant-chart-card
                        id="dashboard-reservations"
                        :title="__('Activité des réservations')"
                        :description="__('Créations et changements d’état observés sur la période du tableau de bord.')"
                        chart="reservations"
                    ...
                    <x-tenant-chart-card
                        id="dashboard-fleet"
                        :title="__('Disponibilité du parc')"
                        :description="__('Situation réelle du parc à la date de référence du rapport.')"
                        chart="fleet"
        ...
        <div class="grid gap-6 xl:grid-cols-2">
            @if ($maintenanceSummary !== null)
                <section class="rf-panel p-5 xl:col-span-2"><div class="flex items-center justify-between gap-3"><h2 class="font-semibold">{{ __('Pilotage maintenance') }}</h2><a href="{{ route('maintenance.index') }}" class="rf-button-link">{{ __('Voir le module') }}</a></div><div class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">@foreach($maintenanceSummary as $label => $value)<x-stat-card :label="$label" :value="App\Support\Ui\BusinessNumber::integer($value)" icon="refresh" />@endforeach</div></section>
            @endif
            @if ($insuranceSummary !== null)
                <section class="rf-panel p-5 xl:col-span-2"><div class="flex items-center justify-between gap-3"><div><h2 class="font-semibold">{{ __('Pilotage assurance') }}</h2><p class="text-sm text-slate-500">{{ __('Alertes administratives uniquement, sans décision juridique automatique.') }}</p></div><a href="{{ route('insurance.index') }}" class="rf-button-link">{{ __('Voir le module') }}</a></div><div class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">@foreach($insuranceSummary as $label => $value)<x-stat-card :label="$label" :value="App\Support\Ui\BusinessNumber::integer($value)" icon="warning" />@endforeach</div></section>
            @endif
            ...
            @if ($unavailableVehicles !== null)
                <section class="rf-panel p-5">
                    <div class="flex items-center justify-between gap-3"><h2 class="font-semibold">{{ __('Véhicules indisponibles') }}</h2><a href="{{ route('vehicles.index', ['status' => 'maintenance']) }}" class="rf-button-link">{{ __('Voir la flotte') }}</a></div>
                    <div class="mt-4 divide-y">
                        @forelse ($unavailableVehicles as $vehicle)<a href="{{ route('vehicles.show', $vehicle) }}" class="flex items-center justify-between gap-3 py-3"><span>{{ $vehicle->registration_number }} · {{ $vehicle->brand }} {{ $vehicle->model }}</span><x-status-badge :value="$vehicle->operational_status" /></a>@empty <x-empty-state :title="__('Toute la flotte est opérationnelle')" /> @endforelse
                    </div>
                </section>
            @endif
            ...
            @if ($unpaidInvoices !== null)
                <section class="rf-panel p-5">
                    <div class="flex items-center justify-between gap-3"><h2 class="font-semibold">{{ __('Factures impayées') }}</h2><a href="{{ route('finance.index') }}" class="rf-button-link">{{ __('Voir la finance') }}</a></div>
                    <div class="mt-4 divide-y">
                        @forelse ($unpaidInvoices as $invoice)<a href="{{ route('finance.invoices.show', $invoice) }}" class="flex items-center justify-between gap-3 py-3"><span><strong>{{ $invoice->invoice_number }}</strong><br><span class="text-sm text-slate-500">{{ __('Échéance') }} {{ App\Support\Ui\UiLabel::date($invoice->due_at) }}</span></span><span class="font-semibold">{{ App\Support\Ui\UiLabel::money($invoice->balance_due, $invoice->currency) }}</span></a>@empty <x-empty-state :title="__('Aucune facture impayée')" /> @endforelse
                    </div>
                </section>
            @endif
            ...
            @if ($upcomingMaintenance !== null)
                <section class="rf-panel p-5">
                    <div class="flex items-center justify-between gap-3"><h2 class="font-semibold">{{ __('Maintenances à surveiller') }}</h2><a href="{{ route('maintenance.index') }}" class="rf-button-link">{{ __('Voir tout') }}</a></div>
                    <div class="mt-4 divide-y">@forelse ($upcomingMaintenance as $maintenance)<a href="{{ route('maintenance.show', $maintenance) }}" class="flex items-center justify-between gap-3 py-3"><span>{{ $maintenance->maintenance_number }} · {{ $maintenance->vehicle->registration_number }}</span><x-status-badge :value="$maintenance->status" /></a>@empty <x-empty-state :title="__('Aucune maintenance proche')" /> @endforelse</div>
                </section>
            @endif

            @if ($openClaims !== null)
                <section class="rf-panel p-5">
                    <div class="flex items-center justify-between gap-3"><h2 class="font-semibold">{{ __('Sinistres ouverts') }}</h2><a href="{{ route('insurance.index') }}" class="rf-button-link">{{ __('Voir l’assurance') }}</a></div>
                    <div class="mt-4 divide-y">@forelse ($openClaims as $claim)<a href="{{ route('insurance.claims.show', $claim) }}" class="flex items-center justify-between gap-3 py-3"><span>{{ $claim->claim_number }}</span><x-status-badge :value="$claim->status" /></a>@empty <x-empty-state :title="__('Aucun sinistre ouvert')" /> @endforelse</div>
                </section>
            @endif
            @if ($expiringPolicies !== null)
                <section class="rf-panel p-5"><h2 class="font-semibold">{{ __('Polices proches de l’échéance') }}</h2><div class="mt-4 divide-y">@forelse($expiringPolicies as $policy)<a href="{{ route('insurance.policies.show',$policy) }}" class="flex items-center justify-between gap-3 py-3 text-sm"><span>{{ $policy->vehicle->registration_number }} · {{ $policy->company->name }}</span><span>{{ App\Support\Ui\UiLabel::date($policy->ends_at) }}</span></a>@empty<x-empty-state :title="__('Aucune échéance proche')" />@endforelse</div></section>
            @endif
            @if ($uninsuredVehicles !== null)
                <section class="rf-panel p-5"><h2 class="font-semibold">{{ __('Véhicules sans police active') }}</h2><div class="mt-4 divide-y">@forelse($uninsuredVehicles as $vehicle)<a href="{{ route('vehicles.show',$vehicle) }}" class="block py-3 text-sm">{{ $vehicle->registration_number }} · {{ $vehicle->brand }} {{ $vehicle->model }}</a>@empty<x-empty-state :title="__('Tous les véhicules actifs sont couverts')" />@endforelse</div></section>
            @endif

            @if ($expiringDocuments !== null || $expiringLicences !== null)
                <section class="rf-panel p-5">
                    <h2 class="font-semibold">{{ __('Échéances documentaires') }}</h2>
                    <div class="mt-4 space-y-4 text-sm">
                        @if ($expiringLicences !== null) @forelse ($expiringLicences as $driver)<div class="rounded-lg border p-3"><strong>{{ __('Permis à renouveler') }}</strong><p class="text-slate-600">{{ $driver->first_name }} {{ $driver->last_name }} · {{ App\Support\Ui\UiLabel::date($driver->licence_expires_at) }}</p></div>@empty @endforelse @endif
                        @if ($expiringDocuments !== null) @forelse ($expiringDocuments as $document)<a href="{{ route('documents.show', $document) }}" class="block rounded-lg border p-3"><strong>{{ $document->title }}</strong><p class="text-slate-600">{{ __('Échéance de conservation ·') }} {{ App\Support\Ui\UiLabel::date($document->retention_until) }}</p></a>@empty @endforelse @endif
                        @if (($expiringLicences?->isEmpty() ?? true) && ($expiringDocuments?->isEmpty() ?? true))<x-empty-state :title="__('Aucune échéance documentaire proche')" />@endif
                    </div>
                </section>
            @endif
